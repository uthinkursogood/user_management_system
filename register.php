...
// form variables
$first_name = $last_name = $email = $gender = $role = '';
$errors = array();
$success_message = '';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate all inputs - required fields must not be empty
    if (empty($first_name)) {
    ...
    } elseif (!passwordsMatch($password, $confirm_password)) {
    ...
    if (empty($errors)) {
        ...
        if (getUserByEmail($conn, $email)) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (first_name, last_name, email, password, gender, role, status) VALUES (?, ?, ?, ?, ?, ?, 'active')");
            if ($stmt) {
                $stmt->bind_param("ssssss", $first_name, $last_name, $email, $hashed_password, $gender, $role);
...
<div class="register-container">
    <div class="register-card">
        ...
        <p class="subtitle">Register as User or Admin</p>
